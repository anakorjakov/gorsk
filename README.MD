...
1. To use Gorsk as a starting point of a real project whose package name is something like `github.com/author/project`, move the directory `$GOPATH/github.com/ribice/gorsk` to `$GOPATH/github.com/author/project`. We use [task](https://github.com/go-task/task) as a task runner. Run ``task relocate`` or ``task relocate-win`` (in Git Bash) to rename the packages, e.g. ``task relocate TARGET=github.com/author/project``, ``task relocate-win TARGET=github.com/anakorjakov/skeleton``
